...
int main(void) {
	...
	for (index = 0; index < loops; index++) {
		output[index] = Factorial(input[index]); //stores our factorial calc but doesn't seem necessary? Homework says to store it
	    printf("%15d %15llu\n", input[index], Factorial(input[index])); // prints a formatted graph of input and output	    
	}
	while (Factorial((step4a+1)) <= USHRT_MAX) { //Calculates largest factorial that will fit into unsigned short
		step4a+=1;
	}
...
unsigned long long Factorial(unsigned int n) { //Factorial function
	int i;
	unsigned long long tValue;
	for (i=1, tValue=1; i<= n; i++) { //Loops until we hit our factorial and stores it into tValue as unsigned long long
		tValue *= i;
		}
	return tValue;
}
